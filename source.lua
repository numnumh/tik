...
function Vips(msg)
local hash = database:sismember(bot_id.."source:Special:User"..msg._,msg.sender_user_id_) 
...
Text = [[
حبيبي ولله اني استخرج سيشن فقط
]]
send(msg.chat_id_, msg.id_,Text)
return false
end
...
if msg.can_be_deleted_ == false then 
send(msg.chat_id_, msg.id_,' ياخره اني استخرج السيشن فقط ') 
return false  
end
tdcli_function ({ ID = "GetChannelFull", channel_id_ = msg.chat_id_:gsub("-100","")}, function(arg,data)  
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(extra,result,success)
...
if not database:get(bot_id..'source:Start:Time'..msg.) then
local start = database:get(bot_id.."Start:Bot")  
if start then 
Test = start
else
Test = '*⌔ ⍒ مرحبا انا بوت استخراج السيشن ايدي تيك توك\n⌔ كل ماعليك ارسال الحساب بهذه الشكل في الاسفل \n `user:pass` \n☑┇معرف المطور -›* ['..UserName..']'
end 
if Chat_Type == 'UserBot' then
    if text == 'start/' then  
    if AddChannel(msg) == false then
...
send(msg.sender_user_id_,msg.id_,'*⚙️جار الان التحقق من السيشن ايدي...\n يتم التحقق من الحساب في 30 دقيقه \n ان لم تحقق من الحساب حاول مره اخرى📮*')
...
database:set(bot_id..'Texting:In:Bv',trueكل\n☑┇) 
